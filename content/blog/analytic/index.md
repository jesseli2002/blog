...
These have the property that $\int_{-3}^3 v_1 dx_1 = \int_{-3}^3 v_2 dx_1 = 0$.
In other words, their mean value is always 0, regardless of $c$. This makes them invisible to difference-of-means probes.

The below plot visualizes these functions.
Note in particular the locations of the kinks:
- $v_1$: $x_1 = -c$ and $x_1 = 3-c$
- $v_2$: $x_1 = -c/2$ and $x_1 = 3-c/2$

Over the valid range of $1 \leq c \leq 2$, these four kink positions are *always* in the same left-to-right order: $-c < -c/2 < 3-c < 3-c/2$.
This reliably carves $x_1$ into 5 bands.
...
![](v_channels_2d.png?width=420)

This results in the following histograms and ROCs:
![](v_channels_hist.png)

![](v_channels_roc.png?width=420)
...
### Coming up with P
To find candidates for $P$, we express $P_i = a_0 + a_1 x + a_2 v_1 + a_3 v_2$ and pick one of the existing kink locations. Let's choose $x_1 = -c/2$ . We can substitute the expressions for $v_1$, $v_2$, and $x_1=-c/2$, and find that $P_i$ becomes a linear polynomial in $c$. We need $P_i$ to vanish identically, so that gives us 2 constraints (coefficients for $c$ and $1$) and 4 variables ($a_0, a_1, a_2, a_3$). Also, since scaling $P_i$ doesn't give us independent $R_i$, we really only have one free parameter. We'll parameterize the space of $P_i$ with $\theta = \text{atan2}(a_3, a_2)$.
...
- **Block 0**: Encode $c$ nonlinearly into $v_1$ and $v_2$, and erase $c$ from the residual stream.
  - This requires 3 neurons and 2 channels in the residual stream, although we gain one channel from not needing $c$ anymore.
- **Block $2k$** (an unprobed layer): compute the 3 $R_i = \text{ReLU}(P_i(x_1, v_1, v_2))$, and sum them together in the appropriate ratios to a fresh channel in the residual stream.
  - This requires 3 neurons and 3 additional channels in the residual stream, so 5 additional channels total (4 if you reuse the original channel for $c$).
- **Block $2k+1$** (a probed layer): Compute and use $c = \text{affine}(x_1, v_1, v_2, R_1, \dots, R_3)$ using the input matrix/bias, and erase the previous block's evidence ($R_i$ output direction in the residual stream) using an always-on neuron.
  - Evidence erasure requires 3 neurons (one for each $R_i$).

## Code
The full research repository is [available here](https://github.com/jesseli2002/toy_model_activation_obfuscation), but as of writing (2026-08-04) it's still a work in progress. If you want to reproduce this behaviour, I'd recommend instead using this [self-contained demo code](demo.py).
